...
## 1.1. Custom Session Label

This is a new feature introduced in FortiManager 8.0.0 (#1220831) that allows administrators to specify a custom session label for better tracking of changes. For example, the custom session label can be set to the ID of a specific change request, and that label can then be searched in the event logs to find all actions and operations performed in FortiManager to address that change request.

### 1.1.1. How to set a custom session label?

The following example shows how to set the custom session label to 
```
SR
```
#00001
...
```
```
### 1.1.2. How to get the custom session label?
```
...
```
```
## 1.2. FortiManager Certificates
```

```
### 1.2.1. How to get FortiManager certificates?
...
#### 1.2.1.1. How to get a FortiManager CA certificate?
...
### 1.2.2. How to get FortiManager local certificates?

The following example shows how to obtain the local certificate named `fmg_76_interim`:
...
The returned certificate is a Certificate Signing Request (CSR) if the `csr` field is not empty. if the `csr` field is empty, it means a signed certificate was returned, and the `certificate` field should be used instead.

### 1.2.3. How to get FortiManager certificates in a human-readable form?
...
### 1.2.4. How to enroll a new FortiManager Certificate?
...
```
# 2. Download the CSR and submit it to your internal PKI for signing.
```

```
# 3. Import the signed certificate back into FortiManager.

#### 1.2.4.1. Create a Certificate Signing Request (CSR)

There’s no FortiManager JSON API endpoint available. You have to use the FortiManager GUI API.

Please refer to [How to generate a FortiManager CSR using FortiManager GUI API?].

#### 1.2.4.2. Download the CSR and submit it to your internal PKI for signing

You can download the CSR using the FortiManager JSON API.

Please refer to [How to get FortiManager local certificates?].

Once, the CSR is downloaded, you can submit it to your internal PKI for signing. Then you will receive a signed certificate back that you will need to import in FortiManager.

#### 1.2.4.3. Import the signed certificate back into FortiManager

There’s no FortiManager JSON API endpoint available. You have to use the FortiManager GUI API.

Please refer to [How to import a signed certificated back using FortiManager GUI API?].

## 1.3. How to get the FortiManager System HA Status?
...
## 1.4. How to get the FortiManager _get system status_?
...
## 1.5. How to get the FortiManager license?
...
### 1.6.1. External Authentication Servers

#### 1.6.1.1. How to create a TACACS+ server?
...
#### 1.6.1.2. How to delete a TACACS+ server?
...
#### 1.6.2.1. How to get user session information?
...
## 1.7. How to create Device Meta fields?
...
## 1.8. How to create an ADOM in a managed FAZ
...
## 1.9. How to get details about the connected API user?
...
## 1.10. Operating the FortiManager system

### 1.10.1. How to reboot FortiManager?
...
### 1.10.2. How to backup the FortiManager?

Caught in #621300.

#### 1.10.2.1. Using REST API
...
This is generating a non encrypted protected archive named `fmg_backup_001.dat`.
...
In this case, resulting backup file `fmg_backup_002.dat` will be encrypted with password `abc123`.

#### 1.10.2.2. Using FortiManager JSON RPC API
...
*   Once the task is completed, you can get your `fmg_backup.dat` FortiManager backup file, from the `tmp` folder of your `10.210.35.207` FTP server

> **Warning:**

*   Starting with FortiManager 7.0.11, 7.2.5 and 7.4.2 (#0959025), it is no longer possible to generate a non encrypted backup file.

### 1.10.3. How to restore the FortiManager?

Caught in #621300.

#### 1.10.3.1. Using REST API to restore the FortiManager
...
#### 1.10.3.2. Using FortiManager JSON RPC API to restore FortiManager

Caught in #0746154.

It is possible to restore a FortiManager system using the FortiManager JSON RPC API form, provided the FortiManager backup file has been uploaded in an external FTP, SCP or SFTP server:
...
### 1.10.4. How to upgrade the FortiManager?

#### 1.10.4.1. Using the FortiManager API

Caught in #1100531 (FMG 7.6.3).

The following example shows how to upgrade the FortiManager. FortiManager will fetch the `image.out` firmware using the `10.0.0.1` TFTP server.
...
You can also use `scp`, `ftp` and `sftp` as the `service`. In this case, you need to provide the following details:
...
#### 1.10.4.2. Using REST API to upgrade your FortiManager unit
...
### 1.10.5. How to get CPU, Memory and Disk usage of FortiManager?
...
### 1.10.6. How to execute top or execute iotop?

#### 1.10.6.1. execute top
...
### 1.11.1. How to get the latest task?

In other words, how do you retrieve the newest (most recently created) task using the FortiManager API?

The following example demonstrates how to fetch the latest task by querying the `/task/task/` endpoint, limiting the result to a single entry, and sorting by ID in descending order:
...
*   `range: [0, 1]`: Get results from index 0 to 1, meaning only the first item will be returned (pagination-like behavior).

*   `sortings: [{"id": -1}]`: Sort by the `id` field in descending order (likely to get the most recent task first).

*   `loadsub: 0`: Don’t load sub-objects. Omit it if you need the details of the task.
...
## 1.12. FortiManager Packet capture

It is possible to make packet capture operations for traffic originating or destined to the FortiManager using the FortiManager API.

### 1.12.1. How to get existing packet capture definitions?
...
### 1.12.2. How to add a new packet capture definition?
...
The `id` field is mandatory. Setting it to `0` prompts FortiManager to automatically assign the next available ID.
...
### 1.12.3. How to start a packet capture?

The following example shows how to start a packet capture from an existing packet capture definition:
...
### 1.12.4. How to restart a packet capture?

_Restart_ retains packets captured before the stop operation and includes newly captured packets in the final result.
...
### 1.12.5. How to stop a packet capture?

The following example shows how to stop a packet capture from an existing packet capture definition:
...
### 1.12.6. How to get the status of the packet captures?
...
The output shows two packet captures: the one with ID `1` is stopped (`running` is `0`) and the one with ID `2` is running (`running` is `1`).

### 1.12.7. How to download a packet capture?
...
For more details, see the section [How to download a FortiManager packet capture?]

## 1.13. How to license a FortiManager-VM?
